        ...
        <div v-if="contactPhone || contactEmail" class="footer-contact-info">
          <p>
            ...
            <span v-if="contactPhone"
              ><i class="fas fa-phone-alt"></i> {{ contactPhone }}</span
            >
            <span v-if="contactEmail" class="ml-2"
              ><i class="fas fa-envelope"></i> {{ contactEmail }}</span
            >
          </p>
        </div>
      </div>
...
onMounted(async () => {
  try {
    ...
    res.data.forEach((setting) => {
      if (setting.key_name === "contact_phone")
        contactPhone.value = setting.value;
      if (setting.key_name === "contact_email")
        contactEmail.value = setting.value;
    });
  } catch (error) {
