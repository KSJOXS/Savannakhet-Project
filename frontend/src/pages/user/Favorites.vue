    ...
    <div class="explore-container">
      ...
      <div v-if="activeTab === 'places'">
        <div class="results-info">
          <h2>
            My Favorites
            <span class="count-badge">{{ favorites.length }} places</span>
          </h2>
        </div>
        ...
        <div v-else class="places-grid">
          ...
          >
            <div class="card-media">
              <img
                :src="getCoverImage(fav.place.image_url)"
                :alt="fav.place.name"
                @error="handleImgError"
              />
              <button
                class="btn-heart active"
                @click.stop="toggleHeart(fav.place.id)"
              >
        ...
        <div v-if="!loading && favorites.length === 0" class="empty-state">
          ...
          ></i>
          <p>
            You haven't saved any favorites yet. Explore and tap the heart icon!
          </p>
          <button @click="router.push('/explore')" class="btn-explore-now">
            Explore Places
          </button>
        </div>
      </div>
      ...
      <div v-else>
        <div class="results-info">
          <h2>
            Saved Itineraries
            <span class="count-badge">{{ itineraries.length }} plans</span>
          </h2>
        </div>
          ...
          >
            ...
            <div class="itinerary-card-content">
              ...
              <div class="itinerary-footer">
                ...
                <span class="view-link"
                  >View Plan <i class="fas fa-chevron-right"></i
                ></span>
              </div>
            </div>
          </div>
        ...
        <div v-if="!loading && itineraries.length === 0" class="empty-state">
          ...
          ></i>
          <p>
            You haven't created any trip plans yet. Use our AI Trip Planner!
          </p>
          <button @click="router.push('/trip-planner')" class="btn-explore-now">
            Plan a Trip
          </button>
        </div>
      </div>
    ...
    >
      <div class="modal-content itinerary-modal" @click.stop>
        <div class="modal-header">
          ...
          <button class="btn-close" @click="closeItineraryModal">
        ...
        <div class="modal-body">
          <div class="itinerary-meta-large">
            <span class="meta-pill"
              ...
              {{ selectedItinerary.days }} Days</span
            >
            <span class="meta-pill"
                ...
                >
                  ...
                  <div class="timeline-content">
                    <div class="slot-label">
                      <i
                        v-if="item.time_slot === 'Morning'"
                        class="fas fa-sun text-warning"
                      ></i>
                      <i
                        v-else-if="item.time_slot === 'Afternoon'"
                        class="fas fa-cloud-sun text-orange"
                      ></i>
                    ...
                    <div
                      class="place-card-mini"
                      @click="$router.push(`/places/${item.place_id}`)"
                    >
                      <div class="place-img-mini">
                        <img
                          :src="getCoverImage(item.place?.image_url)"
                          alt="place"
                        />
                      </div>
