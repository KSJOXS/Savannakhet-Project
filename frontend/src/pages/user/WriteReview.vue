    ...
    <div class="wr-container">
      <div class="ta-review-box">
        <div class="ta-rm-left">
          ...
          <h1 class="ta-rm-title">
            {{ t("review.tellUsTitle") || "Tell us, how was your visit?" }}
          </h1>
          ...
          <div class="ta-place-selector">
            <div v-if="!selectedPlace" class="search-place-wrapper">
              <label
                >📍
                {{
                  t("review.searchLabel") || "Search for the place you visited:"
                }}</label
              >
              <div class="ta-search-input">
                ...
                <input
                  ...
                  :placeholder="
                    ...
                    'Search hotels, restaurants, landmarks...'
                  "
                />
              </div>
              <div
                v-if="searchResults.length > 0"
                class="search-results ta-results"
              >
                <div
                  v-for="p in searchResults"
                  :key="p.id"
                  class="search-item ta-item"
                  @click="selectPlace(p)"
                >
          ...
          <div class="ta-form-section">
            <div class="ta-field" v-if="selectedPlace">
              ...
              <div class="ta-circle-rating">
                <i
                  v-for="s in 5"
                  :key="s"
                  @click="newPost.rating = s"
                  @mouseenter="hoverRating = s"
                  @mouseleave="hoverRating = 0"
                  :class="[
                    (hoverRating || newPost.rating) >= s ? 'fas' : 'far',
                    'fa-circle',
                  ]"
                >
                </i>
                <span
                  class="ta-rating-text"
                  v-if="newPost.rating || hoverRating"
                >
            ...
            <div class="ta-field">
              ...
              <textarea
                ...
                class="ta-textarea"
                :placeholder="
                  t('review.writePlaceholder') ||
                  'Share the details of your experience...'
                "
              ></textarea>
              <div
                class="ta-char-count"
                :class="{ 'text-danger': newPost.comment.length > 1000 }"
              >
                {{ newPost.comment.length }}/1000
            ...
            <div class="ta-field">
              <label>{{
                t("review.addPhotos") || "Add some photos (Optional)"
              }}</label>
              <label class="ta-photo-dropzone">
                ...
                <strong>{{
                  t("review.clickToAdd") || "Click to add photos"
                }}</strong>
              ...
              <label class="ta-checkbox">
                ...
                <span>{{
                  ...
                  `I certify that this review is based on my own
                                    experience and is my genuine opinion.`
                }}</span>
              </label>
...
const submitPost = async () => {
  ...
  try {
    ...
    if (newPost.value.title)
      finalCommentText = `**${newPost.value.title}**\n${finalCommentText}`;
