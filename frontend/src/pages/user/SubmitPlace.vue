    ...
    <div class="submit-page-container">
        <div class="header-section">
            <div class="header-content">
                <button class="btn-back-circle" @click="$router.back()" :title="t('submit.cancel')">
                    <i class="fas fa-arrow-left"></i>
                </button>
                <div class="title-group">
                    <h1>{{ isEditMode ? t('submit.titleEdit') : t('submit.titleAdd') }}</h1>
                    <p class="subtitle">{{ isEditMode ? t('submit.subtitleEdit') : t('submit.subtitleAdd') }}</p>
                </div>
            </div>
        </div>
            ...
            <div v-if="permissionStatus === 'none'" class="permission-state-card">
                <div class="card permission-card">
                    <div class="card-body">
                        ...
                        <button class="btn-request" @click="requestPermission" :disabled="isRequesting">
                ...
                <div class="left-column">
                    <div class="card info-card">
                        <div class="card-header">
                            <i class="fas fa-info-circle"></i> <span>{{ t('submit.generalInfo') }}</span>
                        </div>
                        <div class="card-body">
                            <div class="input-row">
                                <div class="input-group">
                                    <label>{{ t('submit.placeName') }} <span class="text-danger">*</span></label>
                                    <input v-model="form.name" :placeholder="t('submit.namePlaceholder')" required>
                                </div>
                                <div class="input-group">
                                    <label>{{ t('submit.category') }} <span class="text-danger">*</span></label>
                                    <select v-model="form.category_id" required>
                                        <option value="" disabled>{{ t('submit.selectCategory') }}</option>
                                        <option v-for="cat in categories" :key="cat.id" :value="cat.id">
                                            [{{ t('categories.' + (cat.parent_type || 'other')).toUpperCase() }}] {{ cat.name }}
                                        </option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="input-row" v-if="isHotelCategory" style="margin-top: 15px; background: #f0f9ff; padding: 15px; border-radius: 12px; border: 1px solid #bae6fd;">
                                <div style="grid-column: 1 / -1; margin-bottom: 10px; display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <span style="font-weight: 700; color: #0369a1;">{{ t('submit.enableBooking') }}</span>
                                </div>
                                <template v-if="showBookingLinks">
                                    <div class="input-group" style="margin-bottom: 0;">
                                        <label style="color: #0369a1; font-weight: 700;"><i class="fas fa-link"></i> {{ t('submit.bookingUrl') }}</label>
                                        <input v-model="form.booking_url" placeholder="https://www.booking.com/hotel/...">
                                    </div>
                                    <div class="input-group" style="margin-bottom: 0;">
                                        <label style="color: #0369a1; font-weight: 700;"><i class="fas fa-link"></i> {{ t('submit.agodaUrl') }}</label>
                            ...
                            <div class="input-group" style="margin-top: 10px;">
                                <label style="color: #6366f1;"><i class="fas fa-paste"></i> {{ t('submit.pasteMap') }}</label>
                            ...
                            <div class="input-group">
                                <label>{{ t('submit.description') }} <span class="text-danger">*</span></label>
                                <textarea v-model="form.description" rows="4" :placeholder="t('submit.descPlaceholder')" required></textarea>
                            </div>
                            ...
                            <div v-if="isEditMode" class="guide-sections-editor">
                                <div class="gse-header">
                                    ...
                                    <span class="gse-count" v-if="sections.length > 0">{{ t('submit.sectionCount', { count: sections.length }) }}</span>
                                </div>
                                ...
                                <div class="gse-list" v-if="sections.length > 0">
                                    ...
                                    >
                                        ...
                                        <div class="gse-item-bar" @click="toggleExpandSection(sec.id)">
                                            <div class="gse-item-left">
                                                ...
                                                <div class="gse-item-info">
                                                    ...
                                                    <span class="gse-item-desc-preview">{{ (sec.description || '').substring(0, 60) || t('submit.noDescription') }}{{ (sec.description || '').length > 60 ? '...' : '' }}</span>
                                                </div>
                                            </div>
                                            <div class="gse-item-right" @click.stop>
                                                <button type="button" class="gse-btn" @click="moveSectionUp(idx)" :title="t('submit.moveUp')"><i class="fas fa-arrow-up"></i></button>
                                                <button type="button" class="gse-btn" @click="moveSectionDown(idx)" :title="t('submit.moveDown')"><i class="fas fa-arrow-down"></i></button>
                                                <button type="button" class="gse-btn edit" @click="startEditSection(sec)" :title="t('submit.edit')"><i class="fas fa-pen"></i></button>
                                                <button type="button" class="gse-btn danger" @click="deleteSection(sec.id)" :title="t('submit.delete')"><i class="fas fa-trash"></i></button>
                                        ...
                                        <div v-if="expandedSection === sec.id && editingSectionId !== sec.id" class="gse-expand-body">
                                            ...
                                            <div v-if="!sec.image_url" class="gse-no-img"><i class="fas fa-image"></i> {{ t('submit.noImageYet') }}</div>
                                            <p class="gse-desc-text">{{ sec.description || t('submit.noDescription') }}</p>
                                        </div>
                                        ...
                                        <div v-if="editingSectionId === sec.id" class="gse-edit-body">
                                            <label class="gse-upload-area" :class="{ 'has-img': editSectionPreview || sec.image_url }">
                                                ...
                                                <div v-else-if="sec.image_url" class="gse-upload-existing-wrap">
                                                    ...
                                                    <span class="gse-change-hint"><i class="fas fa-camera"></i> {{ t('submit.changeImage') }}</span>
                                                </div>
                                            ...
                                            <textarea v-model="editSectionDesc" rows="5" :placeholder="t('submit.sectionPlaceholder')" class="gse-textarea"></textarea>
                                            <div class="gse-action-row">
                                                <button type="button" class="gse-save-btn" @click="saveEditSection(sec.id)" :disabled="isSavingSection">
                                                    ...
                                                    {{ isSavingSection ? t('submit.saving') : t('submit.saveSection') }}
                                                </button>
                                                <button type="button" class="gse-cancel-btn" @click="cancelEditSection">{{ t('submit.cancel') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div v-if="showAddSection" class="gse-add-form">
                                    <div class="gse-add-title"><i class="fas fa-plus-circle"></i> {{ t('submit.newSection') }}</div>
                                    ...
                                    <textarea v-model="newSectionDesc" rows="5" :placeholder="t('submit.sectionPlaceholder')" class="gse-textarea"></textarea>
                                    <div class="gse-action-row">
                                        <button type="button" class="gse-save-btn" @click="addSection" :disabled="isSavingSection">
                                            <i class="fas" :class="isSavingSection ? 'fa-spinner fa-spin' : 'fa-check'"></i>
                                            {{ isSavingSection ? t('submit.adding') : t('submit.addSection') }}
                                        </button>
                                        <button type="button" class="gse-cancel-btn" @click="showAddSection = false; newSectionPreview = null; newSectionDesc = ''">{{ t('submit.cancel') }}</button>
                                    </div>
                                </div>

                                <!-- + Add Section trigger button -->
                                <button v-if="!showAddSection" type="button" class="gse-add-trigger" @click="showAddSection = true; expandedSection = null">
                    ...
                    <div class="card premium-details-card">
                        <div class="card-header">
                            <i class="fas fa-star"></i> <span>{{ t('submit.premiumDetails') }}</span>
                        </div>
                        <div class="card-body">
                            <div class="input-row">
                                <div class="input-group">
                                    ...
                                    <div v-if="!form.is_year_round" class="month-range fade-in" style="display: flex; gap: 10px; align-items: center;">
                                        <select v-model="form.best_months_start" style="flex: 1; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1;">
                                            <option v-for="m in months" :key="m" :value="m">{{ t('common.' + m) }}</option>
                                        </select>
                                        <span style="color: #94a3b8; font-weight: 600;">{{ t('submit.to') }}</span>
                                    ...
                                    <div v-else style="padding: 10px; background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 8px; color: #64748b; font-weight: 600; text-align: center;">
                                        <i class="fas fa-calendar-check" style="margin-right: 8px;"></i> {{ t('submit.yearRound') }}
                                    </div>
                                </div>
                                <div class="input-group">
                            ...
                            <div class="input-row">
                                <div class="input-group">
                                    ...
                                    <div v-else style="padding: 10px; background: #ecfdf5; border: 1px dashed #10b981; border-radius: 8px; color: #047857; font-weight: 700; text-align: center;">
                                        <i class="fas fa-gift" style="margin-right: 8px;"></i> {{ t('submit.freeEntry') }}
                                    </div>
                                </div>
                                <div class="input-group">
                    ...
                    <div class="card map-card">
                        <div class="card-header">
                            <i class="fas fa-map-marked-alt"></i> <span>{{ t('submit.mapLocation') }}</span>
                        ...
                        <div class="card-body">
                            <div class="status-toggle-box" :class="form.is_published ? 'active' : 'draft'">
                                <div class="toggle-info">
                                    ...
                                    <span>{{ form.is_published ? t('submit.statusPublic') : t('submit.statusDraft') }}</span>
                                </div>
                                <label class="switch">
                            ...
                            <div class="btn-group-vertical">
                                <button type="submit" class="btn-submit-full" :disabled="isSaving">
                                    <i class="fas" :class="isSaving ? 'fa-spinner fa-spin' : (isEditMode ? 'fa-save' : 'fa-paper-plane')"></i> 
                                    {{ isSaving ? (isEditMode ? t('submit.updating') : t('submit.submitting')) : (isEditMode ? t('submit.updateBtn') : t('submit.submitBtn')) }}
                                </button>
...
const loadPlaceData = async () => {
    try {
        ...
        if (p.daily_budget) {
            if (p.daily_budget === 'Free' || p.daily_budget?.toLowerCase().includes('free')) {
                form.value.is_free = true
            } else {
                const match = p.daily_budget.match(/₭ (\d+k?) - (\d+k?)/)
                if (match) {
                    const parseVal = (v) => v.endsWith('k') ? parseInt(v) * 1000 : parseInt(v)
...
.slider:before, .oh-slider:before {
    position: absolute;
    content: "";
    height: 16px;
    width: 16px;
...
input:checked + .slider, input:checked + .oh-slider {
