        ...
        <div class="user-info-card">
          ...
          <nav class="profile-nav">
            ...
            >
              ...
              <span>{{ t("auth.review_history") }}</span>
            </button>
            <button
              class="nav-item"
        ...
        <div v-else class="content-card">
              ...
              <div class="form-actions">
                ...
                <div v-if="isEditing" class="btn-group-profile">
                  ...
                  >
                    ...
                    {{ saving ? t("auth.saving") : t("auth.save_changes") }}
                  </button>
                </div>
          ...
          <div v-if="activeTab === 'reviews'" class="tab-pane fade-in">
            <div class="stats-row-modern">
              ...
              <div class="stat-card-modern">
                <div class="stat-icon place-icon">
            ...
            <div v-if="userReviews.length === 0" class="empty-state">
              ...
              <router-link to="/community" class="btn-primary-gradient">{{
                t("auth.explore_community")
              }}</router-link>
            </div>
                ...
                <div class="post-card-header">
                  <div class="post-place-info">
                    <router-link
                      ...
                      class="post-place-name"
                    >
                      {{ review.place_name }}
                    </router-link>
                    <span class="post-timestamp"
                      ><i class="far fa-clock"></i>
                      {{ formatDate(review.visited_at) }}</span
                    >
                  </div>
          ...
          <div v-if="activeTab === 'places'" class="tab-pane fade-in">
            <div v-if="userPlaces.length === 0" class="empty-state">
              ...
              <router-link to="/submit-place" class="btn-primary-gradient">{{
                t("auth.submit_first_place")
              }}</router-link>
            </div>
                ...
                <div class="place-card-content">
                  ...
                  <div class="place-footer">
                    ...
                    <div class="place-actions">
                      <button
                        @click="router.push(`/submit-place/${place.id}`)"
          ...
          <div v-if="activeTab === 'trips'" class="tab-pane fade-in">
            <div v-if="userItineraries.length === 0" class="empty-state">
              ...
              <router-link to="/trip-planner" class="btn-primary-gradient">{{
                t("auth.start_planning")
              }}</router-link>
            </div>
                  ...
                  <div class="trip-info">
                    ...
                    <div class="trip-meta">
                      <span class="meta-item"
                        ...
                        {{ itinerary.days }} {{ t("tripPlanner.days") }}</span
                      >
                      <span class="meta-separator">•</span>
                      <span class="meta-item"
                        ><i class="far fa-calendar-alt"></i>
                        {{ formatDate(itinerary.created_at) }}</span
                      >
                    </div>
                  </div>
          ...
          <div class="modal-title-group">
            ...
            <div class="itinerary-meta-pills">
              <span class="pill"
                ...
                {{ selectedItinerary.days }} {{ t("tripPlanner.days") }}</span
              >
              <span class="pill"
                ><i class="fas fa-clock"></i>
                {{ formatDate(selectedItinerary.created_at) }}</span
              >
            </div>
          </div>
          <button class="btn-close-modern" @click="closeItineraryModal">
            ...
            >
              ...
              <div class="timeline-items">
                <div
                  ...
                  v-for="item in selectedItinerary.items.filter(
                    (i) => i.day === day,
                  )"
                  :key="item.id"
                >
                  ...
                  <div class="entry-content">
                    ...
                    <div
                      ...
                      @click="handlePlaceClick(item.place_id)"
                    >
                      <img
                        :src="getPlaceImage(item.place?.image_url)"
                        alt="place"
                        class="preview-img"
                      />
                      <div class="preview-info">
                        <h5>
                          {{ item.place?.name || t("auth.unknown_place") }}
                        </h5>
                        <div class="preview-rating">
                          <i class="fas fa-star"></i>
                          {{ item.place?.rating_avg || "0.0" }}
                        </div>
                      </div>
...
const tabTitle = computed(() => {
  switch (activeTab.value) {
    ...
    case "reviews":
      return t("auth.my_review_history");
    case "places":
      return t("auth.places_i_added");
    case "trips":
      return t("auth.my_saved_trips");
    default:
      return t("auth.profile_tab");
  }
});
...
const tabSubtitle = computed(() => {
  switch (activeTab.value) {
    case "settings":
      return t("auth.manage_settings");
    case "reviews":
      return t("auth.review_history_subtitle");
    case "places":
      return t("auth.places_subtitle");
    case "trips":
      return t("auth.trips_subtitle");
    default:
      return "";
  }
});
...
const cancelEdit = () => {
  ...
  form.value = {
    ...
    preferences: originalData.value.preferences
      ? [...originalData.value.preferences]
      : [],
    password: "",
  };
...
const fetchHistory = async (userId) => {
  try {
    ...
    if (placesRes.status === "fulfilled")
      userPlaces.value = placesRes.value.data;
    if (itinerariesRes.status === "fulfilled")
      userItineraries.value = itinerariesRes.value.data;
  } catch (historyErr) {
...
const confirmDeleteItinerary = async (id) => {
  if (confirm(t("auth.delete_trip_confirm"))) {
    try {
      ...
      userItineraries.value = userItineraries.value.filter(
        (it) => it.id !== id,
      );
    } catch (err) {
...
.timeline-entry:last-child .entry-time::after {
  display: none;
}
