    ...
    <div class="nature-page">
        ...
        <div class="nature-header">
            <div class="header-container">
                ...
                <p class="subtitle">{{ t('nature.subtitle') || 'Discover serene landscapes, lush forests, and outdoor adventures.' }}</p>
            ...
            <main class="nature-list-area">
                <div class="list-header">
                    <h2>
                        ...
                        {{ filteredNature.length !== 1 ? t('landmarks.foundLabelPlural') || 'nature spots' : t('landmarks.foundLabel') || 'nature spot' }} {{ t('landmarks.found') || 'found' }}
                    </h2>
                    <div class="sort-by">
                ...
                <div v-else-if="filteredNature.length === 0" class="empty-box">
                    ...
                    <p>{{ t('landmarks.noResults') || 'No nature spots found matching your search.' }}</p>
                ...
                <div v-else class="nature-grid">
                    <div v-for="(place, index) in sortedNature" :key="place.id"
                        class="nature-card" @click="goToDetail(place.id)">
...
const natureCategories = computed(() => {
    return categories.value.filter(c => {
        ...
        return ['nature', 'park', 'forest', 'waterfall', 'mountain'].includes(pType)
    })
})
...
const getIconForNature = (name) => {
    ...
    if (name.includes('waterfall') || name.includes('river') || name.includes('lake')) return 'fas fa-water'
    if (name.includes('mountain') || name.includes('cave')) return 'fas fa-mountain'
...
const handleImgError = (e) => {
    e.target.src = 'https://images.unsplash.com/photo-1469474968028-56623f02e42e?q=80&w=600'
}

const getCategoryName = (id) => categories.value.find(c => c.id === id)?.name || 'Nature'
...
.nature-card:hover .card-img-wrapper img { transform: scale(1.06); }
...
.nature-card:hover .place-name { color: #93c5fd; }
...
.nature-card:hover .btn-details { color: #00aa6c; }
