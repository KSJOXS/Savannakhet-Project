<template>
  <div class="explore-page">
        ...
        <div class="explore-layout">
          <aside class="sidebar">
            ...
            <div
              ...
              @click="router.push('/trip-planner')"
            >
              ...
              <div class="filter-group">
                ...
                <div class="category-list">
                  ...
                  <button
                    ...
                    class="cat-pill show-more-btn"
                  >
                    {{
                      showAllCategories
                        ? "▲ " +
                          (t("recommend.show_less") || "Thu gọn / Show less")
                        : "▼ " +
                          (t("recommend.show_more") || "Xem thêm / Show more")
                    }}
                  </button>
                </div>
              </div>
              <button @click="resetFilters" class="btn-clear">
                {{ t("recommend.clear_filters") }}
              </button>
            </div>
          </aside>
          ...
          <main class="content-area">
            <div class="results-bar">
              <span>{{
                t("recommend.found_places").replace(
                  "{count}",
    ...
    <section
      v-if="favoriteRecommendations.length > 0"
      ...
      <div class="container">
        ...
        >
          <p
            ...
            "
          >
            ❤️ {{ t("recommend.interest_title") || "อ้างอิงจากความสนใจของคุณ" }}
          </p>
          <h2
            style="
              font-size: 2.2rem;
            ...
            "
          >
            {{ t("recommend.interest_title") || "อ้างอิงจากความสนใจของคุณ" }}
          </h2>
          <p style="margin: 6px 0 0; font-size: 1.05rem; color: #64748b">
            {{
              t("recommend.interest_subtitle") ||
              "สถานที่ที่คุณน่าจะชอบจากรายการโปรดของคุณ"
            }}
          </p>
        </div>
        <div class="places-grid">
          <div
            v-for="place in favoriteRecommendations"
            :key="'fav-' + place.id"
            class="ta-card list-card"
            @click="goToDetail(place.id)"
          >
            <div class="card-img-wrapper">
              ...
              <div class="interest-badge">
                ❤️ {{ t("recommend.for_you") || "Recommended" }}
              </div>
              <button
                v-if="!user || user.role !== 'admin'"
                class="btn-heart"
      ...
      <div class="container heritage-container">
        <div class="heritage-images">
          <img
            ...
            alt="Savannakhet Heritage"
            class="heritage-main-img"
          />
          <div class="heritage-float-img">
            <img src="/images/heritage/culture.png" alt="Lao Culture" />
          </div>
        </div>
...
const heroStyle = computed(() => ({
  ...
  backgroundImage:
    heroImages.value.length === 0 ? `url(${DEFAULT_HERO})` : "none",
}));
...
const topRatedPlaces = computed(() => {
  return [...places.value]
    .sort(
      (a, b) =>
        (parseFloat(b.rating_avg) || 0) - (parseFloat(a.rating_avg) || 0),
    )
    .slice(0, 8);
});
...
const fetchData = async () => {
  ...
  try {
    ...
    categories.value = resCats.data.filter(
      (cat, index, self) =>
    ...
    if (user.value && user.value.role !== "admin") {
      ...
      try {
        ...
        const interactionRes = await axios.get(
          `${backendUrl}/api/interactions/user/${user.value.id}/frequent?limit=10`,
        );
        if (interactionRes.data && interactionRes.data.length > 0) {
          recentInteractionCategoryIds.value = [
            ...new Set(
              interactionRes.data.map((item) => item.place.category_id),
            ),
          ];
        }

        const aiRes = await axios.get(
          `${backendUrl}/api/recommendations/${user.value.id}?top_k=12`,
        );
        if (
          aiRes.data.status === "success" &&
          aiRes.data.recommended_places?.length > 0
        ) {
          recommendedPlaces.value = aiRes.data.recommended_places.filter(
          ...
          if (interactionRes.data && interactionRes.data.length > 0) {
            recommendedPlaces.value = interactionRes.data.map((item) => ({
              place: item.place,
              reason:
                t("recommend.viewed_often") || "สถานที่ที่คุณดูบ่อยช่วงนี้",
              score: 0.95,
            }));
          }
        }
        ...
        console.warn(
          "AI Recommendations not ready — using recently viewed instead",
          aiErr,
        );
      }
    }
  ...
  try {
    const resSetting = await settingRepository.getByKey(
      "hero_images_recommend",
    );
    ...
    console.warn(
      "Hero settings for recommend list not found:",
      settingsError?.response?.status || settingsError.message,
    );
  }
};
...
const fetchInterestBasedRecommendations = () => {
  ...
  if (favoriteIds.value.length) {
    ...
    if (likedCategoryIds.length) {
      const catCount = {};
      likedCategoryIds.forEach(
        (cid) => (catCount[cid] = (catCount[cid] || 0) + 1),
      );
      ...
      favoriteRecommendations.value = places.value
        .filter(
          (p) =>
            targetFavoriteCatIds.includes(p.category_id) &&
            !favoriteIds.value.includes(p.id),
        )
        .sort(
          (a, b) =>
            (parseFloat(b.rating_avg) || 0) - (parseFloat(a.rating_avg) || 0),
...
const filteredPlaces = computed(() => {
  ...
  return [...results].sort((a, b) => {
    ...
    const aIsRecent = recentInteractionCategoryIds.value.includes(
      a.category_id,
    );
    const bIsRecent = recentInteractionCategoryIds.value.includes(
      b.category_id,
    );
...
const toggleHeart = async (placeId) => {
  ...
  try {
    ...
    if (res.data.status === "added") {
      ...
      placeRepository
        .logInteraction(user.value.id, placeId, "like")
        .catch(() => {});
    } else {
