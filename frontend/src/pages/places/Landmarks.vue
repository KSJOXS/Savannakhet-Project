<template>
  <div class="landmarks-page">
      ...
      <aside class="filter-sidebar">
        <div class="map-preview" @click="showMapModal = true">
          ...
          <button class="btn-view-map">
            ...
            {{ t("landmarks.viewOnMap") || "View on map" }}
          </button>
        </div>
        ...
        <div class="filter-group">
          ...
          <label class="filter-checkbox"
            ><input type="checkbox" />
            <span>{{ t("landmarks.historyLovers") }}</span></label
          >
          <label class="filter-checkbox"
            ><input type="checkbox" />
            <span>{{ t("landmarks.families") }}</span></label
          >
          <label class="filter-checkbox"
            ><input type="checkbox" />
            <span>{{ t("landmarks.photography") }}</span></label
          >
          <label class="filter-checkbox"
            ><input type="checkbox" />
            <span>{{ t("landmarks.spiritual") }}</span></label
          >
        </div>
      </aside>
      ...
      <main class="landmarks-list-area">
        <div class="list-header">
          <h2>
            ...
            {{
              filteredLandmarks.length !== 1
                ? t("landmarks.foundLabelPlural") || "landmarks"
                : t("landmarks.foundLabel") || "landmark"
            }}
          ...
          >
            ...
            <div class="card-info">
              ...
              <div class="description-snippet">
                <p>
                  {{ place.description || t("landmarks.defaultDescription") }}
                </p>
              </div>
    ...
    <SectionDivider icon="fas fa-gopuram" />
    <RecentlyViewed />
  </div>
</template>
...
const landmarkCategories = computed(() => {
  return categories.value.filter((c) => {
    ...
    return ["landmark", "temple", "museum", "historical"].includes(pType);
  });
});
...
const sortedLandmarks = computed(() => {
  let list = [...filteredLandmarks.value];
...
const getIconForLandmark = (name) => {
  ...
  if (n.includes("temple") || n.includes("wat")) return "fas fa-vihara";
  if (n.includes("museum")) return "fas fa-university";
  if (n.includes("history")) return "fas fa-monument";
  return "fas fa-map-pin";
};

const getCategoryName = (id) => {
  return categories.value.find((c) => c.id === id)?.name || "Landmark";
};
...
const handleImgError = (e) => {
  e.target.src =
    "https://images.unsplash.com/photo-1544257121-628d052a1212?q=80&w=600";
};
...
.landmark-card:hover .card-img-wrapper img {
...
.landmark-card:hover .btn-details {
  color: #00aa6c;
}
