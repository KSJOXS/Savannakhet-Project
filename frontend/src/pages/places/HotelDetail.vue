    ...
    <div v-if="hotel">
      <div class="premium-hero-wrapper">
        ...
        <div
          ...
          v-if="
            ...
            (hotel.avoid_if && hotel.avoid_if.length > 0)
          "
        >
          <div
            class="tag-column best-for"
            v-if="hotel.best_for && hotel.best_for.length > 0"
          >
            <label>AMENITIES / BEST FOR</label>
            <div class="tag-list">
              <span
                v-for="(tag, idx) in hotel.best_for"
                :key="'bf-' + idx"
                class="tag"
                >{{ tag }}</span
              >
          ...
          <div
            ...
            v-if="hotel.avoid_if && hotel.avoid_if.length > 0"
          >
            <label>GOOD TO KNOW / AVOID IF</label>
            <div class="tag-list">
              <span
                v-for="(tag, idx) in hotel.avoid_if"
                :key="'ai-' + idx"
                class="tag"
                >{{ tag }}</span
              >
      ...
      <div class="container-main">
        ...
        <div class="sticky-nav-wrapper" ref="stickyNavRef">
          <div class="sticky-nav" :class="{ 'is-sticky': isSticky }">
            ...
            <div class="nav-action" v-if="isSticky">
              <button class="btn-check-availability" @click="scrollTo('deals')">
                {{ t("hotels.check_availability") }}
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="left-col">
            ...
            <div class="amenities-box">
              ...
              <div class="amenities-grid">
                ...
                <div class="amenity-item">
                  <i class="fas fa-wifi"></i> Free High Speed Internet (WiFi)
                </div>
                <div class="amenity-item">
                ...
                <div class="form-body">
                  ...
                  <div class="star-picker">
                    <i
                      ...
                      :key="s"
                      @click="newRating = s"
                      :class="[newRating >= s ? 'fas' : 'far', 'fa-circle']"
                    ></i>
                  </div>
                  <textarea
                    v-model="newComment"
          ...
          <div class="right-col">
            <div class="booking-widget" id="deals">
              ...
              <div class="partner-deal">
                <img
                  src="https://upload.wikimedia.org/wikipedia/commons/4/41/Booking.com_Logo_2022.png"
                  height="24"
                  alt="Booking.com"
                />
                <div class="partner-info">
                  <span class="partner-name">Booking.com</span>
                  <small class="partner-hint">{{
                    t("hotels.seePrices")
                  }}</small>
                </div>
                <a
                  :href="
                    hotel.booking_url ||
                    `https://www.booking.com/searchresults.html?ss=${hotel?.name || 'Savannakhet'}`
                  "
                  target="_blank"
                  class="btn-partner"
              ...
              <div class="partner-deal">
                <img
                  src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/ce/Agoda_logo.svg/1200px-Agoda_logo.svg.png"
                  height="24"
                  alt="Agoda"
                />
                <div class="partner-info">
                ...
                <a
                  :href="
                    hotel.agoda_url ||
                    `https://www.agoda.com/search?text=${hotel?.name || 'Savannakhet'}`
                  "
                  target="_blank"
                  class="btn-partner"
                  >{{ t("hotels.viewPartnerDeal") }}
              ...
              <p class="free-cancel">
                <i class="fas fa-check"></i> {{ t("hotels.free_cancel") }}
              </p>
            </div>
            ...
            <div class="mini-map-card" id="location">
              ...
              <button class="btn-map-link" @click="openMap">
                {{ t("place.viewOnMap") }}
              </button>
            </div>
          </div>
        ...
        <div
          ...
          v-if="recommendedPlaces && recommendedPlaces.length > 0"
        >
          ...
          <div class="recommended-grid">
            ...
            >
              <div class="rec-img-wrapper">
                ...
                <button class="btn-heart-rec" @click.stop>
...
const galleryImages = computed(() => {
  ...
  const getValidUrl = (rawUrl) => {
    if (!rawUrl || rawUrl === "null" || rawUrl === "undefined")
      return "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1920&q=80";
    let url = rawUrl;
  ...
  if (hotel.value.image_url?.startsWith("[")) {
    try {
      urls = JSON.parse(hotel.value.image_url);
    } catch (e) {
  ...
  try {
    ...
    placeRepository
      ...
      .then((allRes) => {
        ...
        recommendedPlaces.value = allRes.data
          .filter(
            (p) =>
              p.category_id === hotel.value.category_id &&
              p.id !== parseInt(id),
          )
          .slice(0, 4);
      })
    ...
    if (user.value) {
      favoriteRepository
        ...
        .then((favRes) => {
          isFavorite.value = favRes.data.some(
            (f) => f.place_id === parseInt(id),
          );
        })
        .catch((err) => console.error(err));
    }
