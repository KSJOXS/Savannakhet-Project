  ...
  <div class="admin-page">
    ...
    <div class="header-content">
      <div class="title-section">
        ...
        <p class="subtitle">
          Manage global configurations, contact details, and banner images
          across the platform.
        </p>
      </div>
    </div>
    ...
    <div class="settings-layout">
      <div class="card settings-card">
        <div class="card-header">
          <h4>
            <i class="fas fa-address-card"></i> General & Contact Information
          </h4>
        </div>
        <div class="card-body">
          <div class="form-row">
            ...
            <div class="form-group half">
              ...
              <div class="input-wrapper">
                ...
                <input
                  ...
                  placeholder="e.g. info@savannakhet.com"
                />
              </div>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="input-wrapper">
              ...
              <input
                ...
                placeholder="e.g. 🌴 Savannakhet Smart Travel © 2026"
              />
            </div>
          </div>
        </div>
      ...
      <div class="card settings-card hero-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">
            Upload backgrounds for specific pages. (Images auto-save upon
            upload)
          </p>
        </div>
        ...
        <div class="card-body">
          ...
          <div class="tab-content">
            <div class="tab-header-info">
              <h5>
                Background Images for: <strong>{{ activeTabName }}</strong>
              </h5>
              <span
                class="image-count-badge"
                :class="{ full: activeImagesList.length >= 10 }"
              >
            ...
            <div v-if="activeImagesList.length > 0" class="image-gallery">
              <div
                v-for="(url, index) in activeImagesList"
                :key="index"
                class="gallery-item"
              >
              ...
              <div
                v-if="activeImagesList.length < 10"
                class="gallery-item add-slot"
            ...
            <div v-else class="upload-box" @click="triggerFileInput">
              ...
              <span
                >Up to 10 images · JPG, PNG, WEBP · Recommended 1920×1080</span
              >
            </div>
            ...
            <div v-if="isUploading" class="upload-progress">
              ...
              <span
                >Uploading {{ currentUploadIndex }} of
                {{ totalToUpload }}...</span
              >
            </div>
          </div>
    ...
    <div class="master-action-bar">
      <div class="save-info">
        ...
        <span
          >Images are saved automatically. Press save to apply text
          changes.</span
        >
      </div>
      <button
        @click="saveAllSettings"
        class="btn-master-save"
        :disabled="isSaving"
      >
...
const loadSettings = async () => {
  try {
    ...
    res.data.forEach((s) => {
      // Text Settings
      if (s.key_name === "contact_phone")
        settings.contact_phone = s.value || "";
      if (s.key_name === "contact_email")
        settings.contact_email = s.value || "";
      ...
      if (
        ...
        try {
          ...
          if (
            s.key_name === "hero_images_public" ||
            s.key_name === "hero_images"
          )
            imagesHome.value = fullUrls;
          if (s.key_name === "hero_images_recommend")
            imagesRecommend.value = fullUrls;
        } catch (e) {
          console.error(e);
        }
      }
...
const saveAllSettings = async () => {
  ...
  try {
    await Promise.all([
      settingRepository.upsert("contact_phone", {
        value: settings.contact_phone,
      }),
      settingRepository.upsert("contact_email", {
...
const onFilesSelected = async (e) => {
  ...
  for (let i = 0; i < filesToUpload.length; i++) {
    ...
    try {
      ...
      const res = await settingRepository.uploadHeroImage(
        filesToUpload[i],
        activeSettingKey.value,
      );
      if (res.data && res.data.all_images) {
        activeImagesList.value = res.data.all_images.map((url) =>
          getFullImageUrl(url),
        );
      }
    } catch (err) {
...
const removeImage = async (url) => {
  ...
  try {
    ...
    const res = await settingRepository.removeHeroImage(
      pathOnly,
      activeSettingKey.value,
    );
    if (res.data && res.data.all_images) {
      activeImagesList.value = res.data.all_images.map((img) =>
        getFullImageUrl(img),
      );
    }
...
.gallery-item:hover .btn-remove-gallery {
  opacity: 1;
}

.gallery-item.add-slot {
...
.btn-master-save:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 6px 20px rgba(15, 23, 42, 0.3);
}
.btn-master-save:disabled {
  background: #94a3b8;
  cursor: not-allowed;
  box-shadow: none;
}
