      ...
      >
        <div class="modal-box">
          ...
          <div class="modal-head">
            <div
              class="modal-head-avatar"
              :style="getAvatarStyle(selectedMsg.name)"
            >
            ...
            <div class="modal-head-actions">
              <span
                v-if="selectedMsg.is_replied"
                class="status-chip chip-green"
              >
                <i class="fas fa-check-circle"></i> Replied
              </span>
              <span
                v-else-if="selectedMsg.is_read"
          ...
          <div v-if="selectedMsg.is_replied" class="reply-history">
            ...
            <div v-if="selectedMsg.reply_text" class="rh-body">
          ...
          <div class="reply-section" v-if="!selectedMsg.is_replied">
            <div class="reply-header">
              ...
              <div class="reply-destination">
                <i class="fas fa-paper-plane"></i>
                Will be sent to:
                <span class="dest-email">{{ selectedMsg.email }}</span>
              </div>
            </div>
...
const filteredMessages = computed(() => {
  switch (activeFilter.value) {
    ...
    case "read":
      return messages.value.filter((m) => m.is_read && !m.is_replied);
    case "replied":
      return messages.value.filter((m) => m.is_replied);
    default:
...
const getFilterCount = (val) => {
  ...
  if (val === "read")
    return messages.value.filter((m) => m.is_read && !m.is_replied).length;
  if (val === "replied")
    return messages.value.filter((m) => m.is_replied).length;
  return 0;
};
...
const sendReply = async () => {
  ...
  try {
    ...
    errorMsg.value =
      e.response?.data?.detail || "Please check your SMTP settings in .env";
...
.message-row:not(.unread) .sender-name {
  font-weight: 500;
  color: #475569;
}
