<template>
    <div class="admin-page">
        <div class="page-header">
            ...
            <div class="stats-badge" v-if="comments.length > 0">Total: {{ comments.length }} Reviews</div>
        </div>

        <div class="filter-bar">
            <div class="search-box">
                <i class="fas fa-search"></i>
                <input v-model="searchQuery" type="text" placeholder="Search by user, comment or place..." />
            </div>
            <button @click="fetchComments" class="btn-refresh"><i class="fas fa-sync"></i> Refresh Data</button>
        </div>
                    ...
                    <tr v-for="comment in filteredComments" :key="comment.id">
                        <td>
                            <div class="place-cell">
                                <img :src="getThumbnail(comment.place_image || comment.image_url)" class="place-thumb"
                                    @error="e => e.target.src = PLACEHOLDER" />
                                <div class="place-name-group">
                                    <span class="place-name">{{ comment.place_name || 'Place ID: ' + (comment.place_id
                                        || 'Unknown') }}</span>
                                    <button class="btn-view-link" @click="viewPublicPlace(comment.place_id)">
                        ...
                        <td>
                            <div class="user-info">
                                <div class="user-icon">{{ (comment.username || 'U').charAt(0).toUpperCase() }}</div>
                        ...
                        <td>
                            <button @click="handleDelete(comment.id)" class="btn-delete-action">
