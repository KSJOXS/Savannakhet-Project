<template>
  <div class="admin-page">
    <div class="page-header">
      ...
      <button
        class="btn-primary"
        @click="$router.push('/admin/location/create')"
      >
      ...
      <table class="admin-table">
        ...
        <tbody>
          <tr v-for="place in filteredPlaces" :key="place.id">
            <td>
              <img
                :src="getThumbnail(place.image_url)"
                class="thumb-img"
            ...
            <td>
              <button
                @click="$router.push(`/admin/location/update/${place.id}`)"
                class="btn-edit"
              >
...
const filteredPlaces = computed(() => {
  return places.value.filter((place) => {
    ...
    const matchCategory =
      ...
      place.category_id === selectedCategory.value;

    // 3. กรองสถานะ (Status)
    const matchStatus =
      selectedStatus.value === "" ||
      Number(place.is_published) === Number(selectedStatus.value);
