    ...
    <div class="header-section">
      <div class="header-content">
        ...
        <div class="title-group">
          <h1>Edit Place</h1>
          <p class="subtitle">
            Manage details, coordinates, and images for this location.
          </p>
        </div>
      </div>
    </div>
      ...
      <form @submit.prevent="updatePlace" class="form-grid">
        <div class="left-column">
          <div
            ...
            :class="{ 'is-locked': form.is_published }"
          >
                    ...
                    >
                      ...
                      <div class="gse-item-right" @click.stop>
                        <button
                          ...
                          :disabled="idx === 0 || form.is_published"
                          title="Move Up"
                        >
                        ...
                        <button
                          ...
                          :disabled="
                            idx === sections.length - 1 || form.is_published
                          "
                          title="Move Down"
                        >
          ...
          <div
            class="card premium-details-card"
            :class="{ 'is-locked': form.is_published }"
          >
            <div class="card-header">
            ...
            <div class="card-body">
              ...
              <div class="input-group">
                ...
                <div
                  ...
                  :class="{ disabled: form.is_published }"
                >
                  <div class="tag-pills">
                    <span
              ...
              <div class="map-wrapper-large">
                <div v-if="form.is_published" class="map-overlay-locked">
                  <div class="overlay-msg">
                    <i class="fas fa-info-circle"></i> Change to Draft to edit
                    coordinates
                  </div>
                </div>
          ...
          <div class="card upload-card">
            ...
            <div class="card-body">
              <div class="gallery-grid" v-if="images.length > 0">
                ...
                <label
                  v-if="!form.is_published && images.length < 10"
              ...
              <div v-else class="upload-empty-state">
                ...
                <span v-else class="locked-hint"
                  >Change to Draft to upload images</span
                >
              </div>
...
const initMap = () => {
  ...
  marker.value = L.marker([lat, lng], {
    draggable: !form.value.is_published,
  }).addTo(map.value);
...
const fetchDetails = async () => {
  ...
  try {
    ...
    if (
      data.best_months === "Year-round" ||
      data.best_months === "ตลอดทั้งปี"
    ) {
    ...
    if (data.daily_budget) {
      if (
        data.daily_budget === "Free" ||
        data.daily_budget?.toLowerCase().includes("free")
      ) {
...
.btn-sec-action:hover:not(:disabled) {
  background: #f1f5f9;
  color: #1e293b;
}
.btn-sec-action:disabled {
  opacity: 0.35;
  cursor: default;
}
.btn-sec-action.edit:hover {
  background: #eff6ff;
  color: #3b82f6;
  border-color: #93c5fd;
}
...
.sec-upload-area:hover {
  border-color: #6366f1;
}
.sec-upload-area.has-img {
  border-style: solid;
  border-color: #6366f1;
}
...
.sec-upload-placeholder i {
  font-size: 1.8rem;
  color: #cbd5e1;
}
.sec-upload-placeholder small {
  font-size: 0.75rem;
  color: #b8c4d2;
}
...
.sec-upload-area:hover .sec-upload-existing span {
  opacity: 1;
}
...
.btn-sec-save:hover:not(:disabled) {
  background: #4f46e5;
}
.btn-sec-save:disabled {
  opacity: 0.6;
  cursor: default;
}
