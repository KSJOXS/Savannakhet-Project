...
def generate_plot():
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            val_loss = F.binary_cross_entropy_with_logits(
                ...
                torch.cat([torch.ones(pos_val_out.size(0)), torch.zeros(neg_val_out.size(0))])
            )
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1:03d}: Train Loss: {loss.item():.4f}, Val Loss: {val_loss.item():.4f}, Val AUC: {auc:.4f}")
    ...
    ax1.plot(train_losses, label='Train Loss', color='steelblue', linewidth=1.5)
    ax1.plot(val_losses,   label='Validation Loss', color='tomato', linestyle='--', linewidth=1.5)
    ax1.set_xlabel('Epoch', fontsize=12)
    ax1.set_ylabel('Binary Cross-Entropy Loss', fontsize=12)
    ax1.set_title('Learning Curve (Loss)', fontsize=13, fontweight='normal', pad=10)
    ...
    ax2.set_title('Validation AUC over Epochs', fontsize=13, fontweight='normal', pad=10)
