...
def generate_roc_curve():
    ...
    print(
        f"Starting training... Train: {train_size}, Val: {val_size}, Test: {num_edges - train_size - val_size}")
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            val_y_true = torch.cat([torch.ones(pos_val_out.size(
                0)), torch.zeros(neg_val_out.size(0))]).cpu().numpy()
            val_y_score = torch.sigmoid(
            ...
            test_y_true = torch.cat([torch.ones(pos_test_out.size(
                0)), torch.zeros(neg_test_out.size(0))]).cpu().numpy()
            test_y_score = torch.sigmoid(
    ...
    plt.plot(val_fpr, val_tpr, color='blue', lw=2.5,
             label=f'Validation ROC (AUC = {val_roc_auc:.2f})')
    plt.plot(test_fpr, test_tpr, color='darkorange', lw=2.5,
             linestyle='-', label=f'Test ROC (AUC = {test_roc_auc:.2f})')
