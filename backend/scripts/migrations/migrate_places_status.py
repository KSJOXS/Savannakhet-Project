...
def migrate():
    with engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(text(
                "ALTER TABLE places ADD CONSTRAINT fk_places_owner FOREIGN KEY (owner_id) REFERENCES users(id);"))
        ...
        try:
            conn.execute(
                text("ALTER TABLE places ADD COLUMN status VARCHAR(20) DEFAULT 'pending';"))
        ...
        try:
            conn.execute(text(
                "UPDATE places SET status = 'approved' WHERE status IS NULL OR status = 'pending';"))
            conn.commit()
