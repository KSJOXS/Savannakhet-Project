...
def migrate():
    ...
    with engine.connect() as connection:
        ...
        result = connection.execute(
            text("SHOW COLUMNS FROM places LIKE 'booking_url'"))
        if not result.fetchone():
            print("Adding booking_url column...")
            connection.execute(
                text("ALTER TABLE places ADD COLUMN booking_url VARCHAR(500) AFTER avoid_if"))
            connection.commit()
        else:
        ...
        result = connection.execute(
            text("SHOW COLUMNS FROM places LIKE 'agoda_url'"))
        if not result.fetchone():
            print("Adding agoda_url column...")
            connection.execute(
                text("ALTER TABLE places ADD COLUMN agoda_url VARCHAR(500) AFTER booking_url"))
            connection.commit()
        else:
