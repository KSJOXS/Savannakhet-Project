...
# CONFIGURATION
# ════════════════════════════════════════════════════════════════
EPOCHS = 150      # Increased max epochs (Early Stopping will stop it)
HIDDEN_DIM = 64
LR = 0.005        # Reduced for stability
SEED = 42
...
class BaseGNN(torch.nn.Module):
    """
    2-Layer GraphSAGE
    Layer 1: MEAN Aggregation → ReLU + Dropout
...
# --- Edge-Level Random Split (70% Train, 15% Val, 15% Test) ---
...
train_edge_index = pos_edge_index[:, perm[test_size + val_size:]]

n_steps = max(1, train_edge_index.size(1) // 5)
...
for epoch in range(1, EPOCHS + 1):
    ...
    ps, pd = train_edge_index[0], train_edge_index[1]
    ...
    with torch.no_grad():
        ...
        ps_v, pd_v = val_edge_index[0], val_edge_index[1]
        pos_out_val = (out_val["user"][ps_v] * out_val["place"][pd_v]).sum(dim=-1)
        
        neg_ei_val = negative_sampling(
            edge_index=val_edge_index,
            num_nodes=(data["user"].num_nodes, data["place"].num_nodes),
            num_neg_samples=val_edge_index.size(1),
        )
        neg_out_val = (out_val["user"][neg_ei_val[0]] * out_val["place"][neg_ei_val[1]]).sum(dim=-1)
        
        val_labels = torch.cat([torch.ones(pos_out_val.size(0)), torch.zeros(neg_out_val.size(0))]).to(device)
        val_loss = F.binary_cross_entropy_with_logits(torch.cat([pos_out_val, neg_out_val]), val_labels)
        val_losses.append(val_loss.item())
        
        val_correct = (pos_out_val >= 0).float().sum() + (neg_out_val < 0).float().sum()
        val_acc = val_correct.item() / (pos_out_val.size(0) + neg_out_val.size(0))
...
# Restore best model for saving and evaluation
if best_model_state is not None:
    model.load_state_dict(best_model_state)
    print(f"\n[*] Restored best model weights (Val Loss: {best_val_loss:.4f})")
...
ax1.plot(ep, accuracies, color="#2563EB", lw=2, label="Train Acc")
ax1.plot(ep, val_accuracies, color="#059669", lw=2, label="Val Acc")
...
ax2.plot(ep, losses, color="#2563EB", lw=2, label="Train Loss")
ax2.plot(ep, val_losses, color="#DC2626", lw=2, label="Val Loss", linestyle='--')
...
# ────────────────────────────────────────────────────────────────
# 7. Model Evaluation (R@K, N@K, MRR)
# ────────────────────────────────────────────────────────────────
print("\n[*] Calculating evaluation metrics for the report...")

def calculate_metrics(recommendations, ground_truth, k):
    top_k_recs = recommendations[:k]
    
    # Recall
    hits = [1 if item in ground_truth else 0 for item in top_k_recs]
    recall = sum(hits) / len(ground_truth) if len(ground_truth) > 0 else 0.0
    
    # NDCG
    dcg = sum([1.0 / math.log2(i + 2) for i, hit in enumerate(hits) if hit == 1])
...
def test_model(model, data, edge_index):
    model.eval()
    with torch.no_grad():
        out = model(data.x_dict, data.edge_index_dict)
        user_embs = out['user']
        place_embs = out['place']
        
        scores = torch.matmul(user_embs, place_embs.t())
        
        src, dst = edge_index
        ground_truth_dict = {}
        for u, p in zip(src.tolist(), dst.tolist()):
            if u not in ground_truth_dict:
                ground_truth_dict[u] = set()
            ground_truth_dict[u].add(p)
        ...
        for user_id, true_items in ground_truth_dict.items():
            ...
            top_items = torch.argsort(user_scores, descending=True).tolist()
            
            _, _, mrr = calculate_metrics(top_items, true_items, max(k_list))
            mrr_list.append(mrr)
            
            for k in k_list:
                r, n, _ = calculate_metrics(top_items, true_items, k)
                results[k]['recall'].append(r)
                results[k]['ndcg'].append(n)
                
        final_metrics = {}
        for k in k_list:
            final_metrics[f'R@{k}'] = np.mean(results[k]['recall'])
            final_metrics[f'N@{k}'] = np.mean(results[k]['ndcg'])
        final_metrics['MRR'] = np.mean(mrr_list)
        
        return final_metrics

# Đánh giá hiệu năng trên tập Test (15% theo Edge-Level Split)
metrics = test_model(model, data, test_edge_index)

print("\n[RESULTS] KET QUA DANH GIA (Dung de dien vao Word):")
print(f"R@5: {metrics['R@5']:.4f} | R@10: {metrics['R@10']:.4f} | R@20: {metrics['R@20']:.4f}")
print(f"N@5: {metrics['N@5']:.4f} | N@10: {metrics['N@10']:.4f} | N@20: {metrics['N@20']:.4f}")
