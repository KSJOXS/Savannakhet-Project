...
def build_gnn_graph(db: Session):
    ...
    if edges:
        ...
        data['user', 'interacts_with', 'place'].edge_weight = torch.tensor(
            edge_weights, dtype=torch.float)

        # Add reverse edges for undirected message passing
        edges_rev = [[e[1], e[0]] for e in edges]
        data['place', 'rev_interacts_with', 'user'].edge_index = torch.tensor(
            edges_rev, dtype=torch.long).t().contiguous()
        data['place', 'rev_interacts_with', 'user'].edge_weight = torch.tensor(
            edge_weights, dtype=torch.float)
    else:
        data['user', 'interacts_with', 'place'].edge_index = torch.empty(
            (2, 0), dtype=torch.long)
        data['user', 'interacts_with', 'place'].edge_weight = torch.empty(
            (0,), dtype=torch.float)
        data['place', 'rev_interacts_with', 'user'].edge_index = torch.empty(
            (2, 0), dtype=torch.long)
        data['place', 'rev_interacts_with', 'user'].edge_weight = torch.empty(
            (0,), dtype=torch.float)
