...
def get_setting(key_name: str, db: Session = Depends(get_db)):
    ...
    setting = db.query(models.SiteSetting).filter(models.SiteSetting.key_name == key_name).first()
    if not setting:
...
async def upload_hero_image(
    ...
    if len(current_images) >= 10:
        raise HTTPException(status_code=400, detail="Maximum 10 hero images allowed. Please remove one first.")
...
def remove_hero_image(
    ...
    if not setting and key_name == "hero_images_public":
        setting = db.query(models.SiteSetting).filter(models.SiteSetting.key_name == "hero_images").first()

    if not setting or not setting.value:
        raise HTTPException(status_code=404, detail="No hero images found for this category")
    ...
    target_image = next((img for img in current_images if image_url in img), None)
