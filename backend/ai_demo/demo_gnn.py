...
def visualize_graph(data, user_map, place_map, db):
    ...
    nx.draw(G, pos, with_labels=True, node_color=colors,
            node_size=2500, font_size=9, edge_color='gray', width=1.5)

    plt.title(
        "Savannakhet Project - Graph Visualization (dee highlighted in orange)")
...
def run_real_demo():
    ...
    try:
        ...
        model = to_hetero(BaseGNN(hidden_channels=32),
        ...
        with torch.no_grad():
            ...
            test_user_id = target_user.id if (
                target_user and target_user.id in user_map) else list(user_map.keys())[0]

            mapped_user = user_map[test_user_id]
            user_name = db.query(User).filter(
                User.id == test_user_id).first().username
            ...
            print(
                f"  • Places already reviewed/visited (filtered): {already_visited_matching}")
            print(
                f"  • New places matching preferences: {total_matching - already_visited_matching}")

            if total_matching > 0 and already_visited_matching == total_matching:
                print(
                    f"\n  [!] You have already visited all places in the category {prefs}!")
                print(
                    f"      The system is scanning for alternative options from your Taste Profile.")
            ...
            for idx, (v, i) in enumerate(zip(top_v, top_i), 1):
                ...
                avg_rating = float(
                    place_obj.rating_avg) if place_obj and place_obj.rating_avg else 0.0
                location = place_obj.location_name if place_obj and place_obj.location_name else "Savannakhet"
                desc_text = place_obj.description.strip(
                ) if place_obj and place_obj.description else "No description available"
                ...
                print(f"\n {idx}. 📍 {place_name}")
                print(
                    f"    ⭐ Average Rating: {avg_rating:.2f}/5.0 | 📌 Location: {location} | 📂 Category: {cat_name}")
                print(
                    f"    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                print(f"    💖 Overall Match: {overall_pct:.2f}%")
                print(
                    f"      ├─ 👥 GNN Social Match: {gnn_pct:.2f}%  (Weight 60%)")
                print(
                    f"      └─ 🎯 Preference Match: {content_pct:.2f}%  (Weight 40%)")

                # Reasoning explanation
                reasons = []
                if cat_name in prefs:
                    reasons.append(
                        f"it matches the category you selected ('{cat_name}')")
                if gnn_pct > 70:
                    reasons.append(
                        f"you are highly likely to like it based on travel behavior of similar users")
                elif gnn_pct > 50:
                    reasons.append(
                        f"it aligns with general system taste profiles")

                if reasons:
                    reason_str = " and ".join(reasons)
                    print(
                        f"    ℹ️  Reasoning: The system recommends this because {reason_str}")
                else:
                    print(
                        f"    ℹ️  Reasoning: Recommended based on your AI-evaluated Taste Profile")
